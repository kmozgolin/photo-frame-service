...
class ColorWheelView @JvmOverloads constructor(
    ...
    private fun buildWheelBitmap(w: Int, h: Int) {
        ...
        val hsv = floatArrayOf(0f, 0f, 1f) // always V=1 — no black overlay bug
        for (y in 0 until h) {
            for (x in 0 until w) {
                val dx = x - centerX
                val dy = y - centerY
    ...
    // Returns V so dialog can set brightness slider accordingly
    fun setColor(color: Int): Float {
        ...
        saturation = hsv[1]
        invalidate()
        return hsv[2].coerceIn(0.05f, 1f) // floor at 0.05 so black doesn't lock slider at 0
    }
